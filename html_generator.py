...
def generate_dashboard(data):
    ...
    # 信号样式
    signal_text = data.get('signal_text', '观望')
    if '买' in signal_text or '多' in signal_text:
        signal_class = 'buy'
        signal_emoji = '📈'
    elif '卖' in signal_text or '空' in signal_text:
        signal_class = 'sell'
        signal_emoji = '📉'
    else:
        signal_class = 'hold'
...
<div class="container">
    ...
    <div class="header">
        <h1>⛁ 黄金 EA 监控</h1>
        <div class="badge">🤖 AI 置信度 {data['up_probability']}%</div>
    </div>

    <div class="price-card">
        <div class="price-left">
            <div class="label">XAUUSD 实时价格</div>
            <div class="value">{data['price']:.2f} <small>USD</small></div>
        </div>
        <div class="price-right">
            <div class="change {change_class}">{arrow} {change:.2f} ({change_percent:.2f}%)</div>
            <div class="sub">参考价 {data['open_price']:.2f}</div>
        </div>
    </div>

    <div class="grid-4">
        <div class="stat-box"><div class="num">{data['ma20']:.2f}</div><div class="lbl">📈 20日均线</div></div>
        <div class="stat-box"><div class="num">{data['rsi']:.1f}</div><div class="lbl">📊 RSI <i>({rsi_status})</i></div></div>
        <div class="stat-box"><div class="num">{data['bb_width']:.2f}</div><div class="lbl">📉 布林带宽</div></div>
        <div class="stat-box"><div class="num">{data['atr']:.2f}</div><div class="lbl">⚡ ATR 波动率</div></div>
    </div>

    <div class="signal-card">
        <div class="signal-row">
            <span class="signal-label">📌 交易建议</span>
            <span class="signal-value {signal_class}">{signal_emoji} {signal_text}</span>
        </div>
        <div class="signal-reason">⚠️ {data['signal_reason']} &nbsp;·&nbsp; 上涨概率 {data['up_probability']}%</div>
    </div>

    <div class="condition-box">
        <div class="condition-item">
            <div class="tag">📈 建议买入</div>
            <div class="val green">突破 {data['buy_level']:.2f} 做多</div>
        </div>
        <div class="condition-item">
            <div class="tag">📉 建议卖出</div>
            <div class="val red">跌破 {data['sell_level']:.2f} 做空</div>
        </div>
    </div>
    ...
    print("=" * 50)
    print("✅ dashboard.html 已自动生成！")
    print("=" * 50)
    print(f"🕒 更新时间：{now}")
    print(f"💰 当前价格：{data['price']:.2f}")
    print(f"📊 交易信号：{signal_text}")
    print("=" * 50)
    print("📁 右键 dashboard.html → Open with Live Server")
    print("=" * 50)